...
export default function HostCTA() {
  return (
    <section id="hosting" className="page-shell py-14">
      <div className="grid overflow-hidden rounded-[2rem] bg-airbnb-ink text-white lg:grid-cols-[0.9fr_1.1fr]">
        <div className="p-8 sm:p-10 lg:p-12">
          ...
          <button className="mt-7 rounded-full bg-white px-5 py-3 text-sm font-bold text-airbnb-ink transition hover:bg-neutral-100">
            Start hosting
          </button>
        </div>
        <div className="grid gap-4 bg-white/5 p-5 sm:grid-cols-3 lg:p-6">
          {benefits.map((benefit) => {
            const Icon = benefit.icon;
            return (
              <article key={benefit.title} className="rounded-2xl bg-white p-5 text-airbnb-ink">
                <Icon size={24} className="text-airbnb-coral" />
                <h3 className="mt-4 font-bold">{benefit.title}</h3>
                <p className="mt-2 text-sm leading-6 text-airbnb-muted">{benefit.copy}</p>
              </article>
            );
          })}
        </div>
